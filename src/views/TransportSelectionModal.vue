  ...
  <div v-if="isOpen" class="modal-overlay" @click.self="$emit('close')">
    <div class="modal-content fade-in">
      <div class="modal-header">
        <h2 class="modal-title">운송 업체 선택</h2>
          ...
          >
            <div class="item-info">
              ...
              <span class="company-details">
                {{ company.manager || '담당자 미지정' }} | {{ company.officePhone }}
              </span>
            </div>
            <div class="item-region">
              <span class="region-badge">{{ getRegionLabel(company.usableRegion) }}</span>
            </div>
          </div>
        </div>
      </div>
...
const filteredCompanies = computed(() => {
  // 활성 상태인 업체만 필터링 (부모에서 이미 필터링해서 넘겨주겠지만 한 번 더 확인)
  const activeCompanies = props.companies.filter(c => c.status === 'ACTIVE' || !c.status || c.status === 'active')
  
  if (!searchQuery.value) return activeCompanies
...
.company-name { font-weight: 600; color: #0f172a; font-size: 0.95rem; }
.company-details { font-size: 0.8rem; color: #64748b; }
