...
const totalQty = computed(() => salesItems.value.reduce((s, i) => s + i.quantity, 0))
const totalSales = computed(() => salesItems.value.reduce((s, i) => s + i.totalPrice, 0))
  ...
  <div class="content-wrapper">
    ...
    <div class="page-header">
      <div class="header-left">
        <div>
          ...
          <p class="page-desc">{{ selectedMonth }} 월별 전체 상품 매출 현황입니다.</p>
        </div>
      </div>
    </div>
    ...
    <div class="data-table-card">
      <div class="table-header">
        <h3>매출 현황</h3>
        <span class="badge">{{ salesItems.length }}개 상품</span>
      </div>
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="(item, idx) in salesItems" :key="idx">
            <td class="rank-cell">{{ idx + 1 }}</td>
            <td>{{ item.productName }}</td>
            <td class="text-right">{{ item.quantity }}</td>
            <td class="text-right">₩ {{ fmt(item.unitPrice) }}</td>
            <td class="text-right"><strong>₩ {{ fmt(item.totalPrice) }}</strong></td>
          </tr>
          <tr class="total-row">
            <td></td>
