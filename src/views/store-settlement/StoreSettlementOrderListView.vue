...
const fetchData = async () => {
    ...
    try {
        ...
        if (activeTab.value === 'daily') {
            res = await franchiseSettlementsApi.getDailyOrderItems(selectedDate.value)
        } else {
            res = await franchiseSettlementsApi.getMonthlyOrderItems(selectedMonth.value)
        }
...
const totalQty = computed(() => orderItems.value.reduce((s, i) => s + i.totalQuantity, 0))
const totalAmount = computed(() => orderItems.value.reduce((s, i) => s + i.totalAmount, 0))
  ...
  <div class="content-wrapper" :class="{ 'is-loading': isLoading }">
    ...
    <div class="page-header">
      <div class="header-left">
        <div>
          ...
          <p class="page-desc">{{ activeTab === 'daily' ? selectedDate : selectedMonth }} 발주내역입니다.</p>
        </div>
      </div>
    </div>
    ...
    <div class="data-table-card">
      <div class="table-header">
        <h3>발주 상세 내역</h3>
        <span class="badge">{{ orderItems.length }}개 항목</span>
      </div>
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="item in orderItems" :key="item.productName">
            ...
            <td class="text-right"><strong>₩ {{ fmt(item.totalAmount) }}</strong></td>
          </tr>
          <tr v-if="orderItems.length === 0 && !isLoading">
              <td colspan="5" class="empty-cell">발주 내역이 없습니다.</td>
          </tr>
          <tr class="total-row" v-if="orderItems.length > 0">
            <td></td>
